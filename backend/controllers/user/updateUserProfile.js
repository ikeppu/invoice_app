...
const updateUserProfile = asyncHandler(async (req, res) => {
  ...
  if (password || passwordConfirm) {
    res.status(400);
    throw new Error(
      `This routes is not for passwords update please use the password reset functionality instead`
    );
  }

  if (email || isEmailVerified || provider || roles || googleID) {
    res.status(400);
    throw new Error(`You are not allowed to update that fields on this routes`);
  }
